...
이메일 인증은 인증번호 상태, 재발급 쿨다운, 실패 횟수, 회원가입·비밀번호 변경과 메일 사업자
호출을 함께 다룹니다. 서로 다른 저장소와 비동기 실행기를 하나의 업무처럼 묶으면 다음 문제가 생깁니다.
...
`email_verification_scopes`는 정규화 이메일과 용도 조합별 직렬화 지점입니다. 이 행을
`SELECT FOR UPDATE`로 잠근 뒤에만 새 Challenge를 발급합니다. 행이 없을 때는
`INSERT ... ON CONFLICT DO NOTHING`으로 생성한 후 잠그므로 최초 동시 요청도 같은 기준으로
직렬화됩니다.
...
요청 검증
  → PostgreSQL Transaction
      ...
      새 Challenge(PENDING)와 다음 발급 가능 시각 저장
    Commit
  → Resend 동기 호출
...
때만 쿨다운을 해제합니다. 오래 걸린 과거 요청이 최신 Challenge의 쿨다운을 해제할 수 없습니다.
...
인증 성공 시 회원가입 또는 비밀번호 변경과 Challenge `CONSUMED` 전이는 하나의 PostgreSQL
...
- 재발급 쿨다운: `EMAIL_VERIFICATION_COOLDOWN` (`PT1M` 권장)
