...
class AccountRegistrationServiceTest {
    ...
    @Test
    @DisplayName("v1은 OTP 없이 기존 가입 정책·비밀번호 Hash·계정 저장 수행")
    void signsUpWithoutEmailOtp() {
        AccountRepository repository = mock(AccountRepository.class);
        PasswordHasher hasher = mock(PasswordHasher.class);
        given(hasher.hash("password-passphrase")).willReturn("encoded-password");
        given(repository.create(any(Account.class))).willAnswer(invocation -> invocation.getArgument(0));
        AccountRegistrationService service = new AccountRegistrationService(repository, hasher);

        Account account = service.signUp("  USER@Example.com  ", "password-passphrase", " 사용자 ");

        then(account.getEmail()).isEqualTo("user@example.com");
        then(account.getName()).isEqualTo("사용자");
